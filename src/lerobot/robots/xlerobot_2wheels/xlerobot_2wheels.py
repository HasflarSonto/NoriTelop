...
class XLerobot2Wheels(Robot):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        # Configure motors immediately after connect
        # Return_Delay_Time=20 (40µs) — fast but gives CH343 USB adapter
        # time to switch direction on the half-duplex bus
        self.bus1.configure_motors(return_delay_time=20)
        self.bus2.configure_motors(return_delay_time=20)
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        left_arm_pos = self.bus1.sync_read("Present_Position", self.left_arm_motors, num_retry=5)
        right_arm_pos = self.bus2.sync_read("Present_Position", self.right_arm_motors, num_retry=5)
        head_pos = self.bus1.sync_read("Present_Position", self.head_motors, num_retry=5)
        base_wheel_vel = self.bus2.sync_read("Present_Velocity", self.base_motors, num_retry=5)
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.max_relative_target is not None:
            ...
            present_pos_left = self.bus1.sync_read("Present_Position", self.left_arm_motors, num_retry=5)
            present_pos_right = self.bus2.sync_read("Present_Position", self.right_arm_motors, num_retry=5)
            present_pos_head = self.bus1.sync_read("Present_Position", self.head_motors, num_retry=5)
        ...
        # Write Goal_Position individually (sync_write causes bus failures)
        for name, val in left_arm_pos_raw.items():
            self.bus1.write("Goal_Position", name, val, num_retry=5)
        for name, val in right_arm_pos_raw.items():
            self.bus2.write("Goal_Position", name, val, num_retry=5)
        for name, val in head_pos_raw.items():
            self.bus1.write("Goal_Position", name, val, num_retry=5)
        if base_wheel_goal_vel:
            self.bus2.sync_write("Goal_Velocity", base_wheel_goal_vel, num_retry=5)
        return {
            **left_arm_pos,
            **right_arm_pos,
            **head_pos,
